...
يلا حالًا هو سوق محلي للتجارة والتوصيل. يقدر العميل يطلب من تاجر واحد أو من أكثر من تاجر في نفس عملية الشراء، حتى الحد الذي تحدده الإدارة. كل تاجر يحضّر الجزء الخاص به فقط، والسائق المعتمد يستلم الأجزاء النشطة ويوصل الطلب للعميل. الإدارة تتحكم في قواعد المنصة: الموافقات، التصنيفات، المناطق، رسوم التوصيل، العمولات، الفواتير، الفلاش سيلز، الكوبونات، نقاط الكاش باك، التقارير، الأمان والدعم.
...
يحافظ هذا الملف على التقسيم الأصلي إلى جزئين. تم دمج القرارات النهائية داخل أقسامها الصحيحة، بدون تحديثات مراحل أو ملاحق أو نسخ مكررة من نفس الميزة. وجود المتطلب داخل الـPRD يحدد هدف المنتج النهائي، لكنه لا يثبت وحده أن الـRuntime أو الواجهة قد تم تنفيذهما.
...
المنصة تدعم أيضًا الخصومات، نقاط الكاش باك، خصم عيد الميلاد، دعوة عميل لعميل بنقاط، الطلبات الأسبوعية، فواتير التجار الأسبوعية وتسوية السائق اليومية الإجبارية، الفلاش سيلز، رفع إثبات التسوية، شات الدليفري مع العميل، زر Alert/Ring داخل الشات، زر SOS للطوارئ، الإشعارات، تقييم تجربة منتجات التاجر، ومراقبة الإدارة، وتحديد مواقع العميل والتاجر باستخدام Google Maps Platform، وحساب المسافة والمدة ووقت الوصول عبر Google Routes API، وتتبع الدليفري أثناء التوصيل بعد الإسناد.
...
| ---: | ---: | ---: |
| الطلب وCheckout، رصيد وتاريخ نقاط الكاش باك، حالة الطلب، الطلبات الأسبوعية، الشات والدعم. | نعم | تطبيق العميل |
| التسجيل، الطلبات، المنتجات، التصنيفات، العروض، الفلاش سيلز، الفواتير والداشبورد. | نعم | تطبيق التاجر |
| التسجيل، الطلبات المتاحة، خطوات الاستلام والتسليم، الفواتير، الداشبورد، وشات الدليفري مع العميل. | نعم | تطبيق السائق |
...
| ---: | ---: | ---: |
| التسجيل، التحقق عبر واتساب مخصص أو SMS من WhySMS المخصص للعميل عند الإتاحة، حفظ العناوين، التصفح، المفضلة، ملاحظات المنتجات، Checkout، الكوبونات، نقاط الكاش باك، خصم عيد الميلاد عند الاستحقاق، متابعة الطلب لايف، شات الدليفري، تقييم تجربة منتجات كل تاجر بعد التسليم، والطلب الأسبوعي. | الطلب بسهولة والتوفير | العميل |
...
| ضبط رقم طوارئ يلا حالًا، تفعيل SOS، مدة وحدود تكرار Alert/Ring داخل الشات، وإجراءات التعامل مع الحوادث. يظل SOS منفصلًا عن تنبيه العميل. | الأمان والتنبيهات |
...
| زر SOS داخل خريطة الدليفري ينبه المنصة ويبدأ اتصالًا برقم طوارئ يلا حالًا الذي تحدده الإدارة. | SOS / الطوارئ |
...
زر SOS إجراء أمان منفصل داخل خريطة الملاحة، ولا يستخدم قناة تنبيه العميل أو قواعد الـCooldown الخاصة بها.
...
| SOS داخل الخريطة منفصل عن Alert/Ring داخل شات العميل ويستخدم رقم الطوارئ الذي تحدده الإدارة. | الأمان |
...
| الرصيد والسجل وعمليات Reserve وCommit وRestore وReverse. | نقاط الكاش باك |
...
| SOS داخل الخريطة، رقم الطوارئ، إظهار رقم العميل، حدود DND، واختيار مكان التسليم. | الأمان والخصوصية |
| WhySMS وWhatsApp OTP وFirebase وGoogle Maps/Routes والميديا المحمية على Hostinger. | التكاملات والميديا |
...
| دعم التوصيل المجاني. | لو الطلب مؤهل، يطبق النظام الأقل بين الرسوم المحسوبة ولقطة حد الدعم، ويدفع العميل أي فرق. |
...
| فصل SOS عن Alert/Ring، رقم الطوارئ ومدة التكرار من الإدارة، حدود DND، إخفاء رقم العميل، وتأكيد `door` أو `building_entrance`. | الأمان والخصوصية والتسليم |
...
| الشكل المقبول هو Top 10 عام حسب إجمالي النقاط المؤهلة المكتسبة طوال عمر الحساب، مع إظهار مركز صاحب الحساب إذا لم يكن ضمن العشرة. طلب مكافأة مشاركة Facebook/Instagram يستخدم كارت مولدًا من النظام، وإثباتًا/رابطًا مرفوعًا، وMention واضحًا لصفحة يلا حالًا، ومراجعة من الإدارة، ونقاطًا قابلة للضبط، وCooldown، ومنع التكرار. لا ينشئ أي علاقة بين العملاء. لا يصبح حقيقة Runtime إلا بعد اعتماد تجاري مكتوب وتنفيذه. | مقترح تغيير مقبول وغير منفذ | لوحة مراكز محدودة ومكافأة مشاركة موثقة |
| يجيب فقط من FAQ أو قاعدة معرفة تعتمدها الإدارة، ويذكر/يربط مصدر الإجابة، ويتوقف ويحوّل للدعم عند غياب المعلومة. لا يغير طلبًا أو رصيدًا أو حسابًا أو تسوية أو صلاحية. استهلاك مزود الذكاء الاصطناعي تكلفة تشغيلية منفصلة عن سعر التطوير. | إضافة مدفوعة ممكنة وغير منفذة | مساعد AI للـFAQ بوضع القراءة فقط |
...
| الـAvatar وXP والمستويات والرتب والإنجازات والتحديات والصناديق والعجلة والمواسم والمقتنيات وإنشاء الشخصية تحتاج خدمة اقتصاد/محتوى وAsset Pipeline يعمل على كل التطبيقات. نظام النقاط الحالي لا يعني أنها متضمنة. | خارج النطاق | Yalla Identity الكامل وإنشاء الشخصيات |
...
| الحقيقة الحالية هي الكوبونات والفلاش سيل وعروض المنتجات والبانرات والإشعارات. التقسيم الديناميكي والثيمات الموسمية والعجلة والتحديات وقواعد الحملات غير المحدودة وربط أنشطة متعددة منتج حملات مستقل. | خارج النطاق | Campaign Engine ذكي حسب الوقت |
...
| زر SOS ورقم طوارئ الإدارة وتنبيه المنصة وبدء الاتصال متضمنة. حالات «العميل لا يرد»، وإدارة Incident كاملة، وإعادة إسناد دليفري، والتعويض الآلي، وكونسول طوارئ توسعة مدفوعة مستقلة. اكتشاف الحالات بالـAI غير نشط. | متضمن جزئيًا | إدارة حالات الطوارئ الموسعة |
